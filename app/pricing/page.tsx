...
export default function PricingPage() {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 max-w-6xl mx-auto items-stretch">
            ...
            <Card className="p-8 hover:shadow-xl transition-all duration-300 border-2 hover:border-blue-200 flex flex-col h-full">
              <CardHeader className="text-center pb-8 flex-shrink-0">
                ...
                <div className="text-4xl font-bold text-blue-600 mt-6">€69 <small>/ {getTranslation(currentLang, "month").toLowerCase()}</small></div>
            ...
            <Card className="p-8 border-4 border-blue-600 hover:shadow-xl transition-all duration-300 relative scale-105 flex flex-col h-full">
              ...
              <CardHeader className="text-center pb-8 flex-shrink-0">
                ...
                <div className="mt-6">
                  ...
                  <div className="text-4xl font-bold text-blue-600 text-center mt-2">€ 695.52 <small>/ {getTranslation(currentLang, "year").toLowerCase()}</small></div>
                </div>
            ...
            <Card className="p-8 hover:shadow-xl transition-all duration-300 border-2 hover:border-blue-200 flex flex-col h-full">
              <CardHeader className="text-center pb-8 flex-shrink-0">
                ...
                <div className="text-4xl font-bold text-blue-600 mt-6">€2  <small>/ {getTranslation(currentLang, "month").toLowerCase()}</small></div>
  ...
  <div className="bg-blue-50 rounded-lg p-8 mb-8">
    <h3 className="text-xl font-semibold text-gray-900 mb-4">
      💡 {getTranslation(currentLang, "pricingNotes")}
    </h3>
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-left">
      <div>
        <h4 className="font-semibold text-gray-900 mb-2">{getTranslation(currentLang, "taxesTitle")}</h4>
        <p className="text-gray-600 text-sm">
          {getTranslation(currentLang, "taxesDescription")}
        </p>
      </div>
      <div>
        <h4 className="font-semibold text-gray-900 mb-2">{getTranslation(currentLang, "paymentMethodsTitle")}</h4>
        <p className="text-gray-600 text-sm">
          {getTranslation(currentLang, "paymentMethodsDescription")}
        </p>
      </div>
      <div>
        <h4 className="font-semibold text-gray-900 mb-2">{getTranslation(currentLang, "cancellationTitle")}</h4>
        <p className="text-gray-600 text-sm">
          {getTranslation(currentLang, "cancellationDescription")}
        </p>
      </div>
      <div>
        <h4 className="font-semibold text-gray-900 mb-2">{getTranslation(currentLang, "supportTitle")}</h4>
        <p className="text-gray-600 text-sm">
          {getTranslation(currentLang, "supportDescription")}
        </p>
      </div>
    </div>
  </div>
        ...
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center max-w-2xl mx-auto">
            <h2 className="text-3xl md:text-4xl font-bold text-gray-900 mb-6">
              {getTranslation(currentLang, "getStartedNow")}, {getTranslation(currentLang, "noRisk")}
            </h2>
            <p className="text-xl text-gray-600 mb-8">
              {getTranslation(currentLang, "testWithoutRisk")}
            </p>
            <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/contact">
              <button className="text-lg px-8 py-3 bg-transparent border rounded">
                {getTranslation(currentLang, "contactSalesTeam")}
              </button>
            </Link>
              
            </div>
            <p className="text-sm text-gray-500 mt-4">
              {getTranslation(currentLang, "noCreditCard")} • {getTranslation(currentLang, "cancelAnytime")} • {getTranslation(currentLang, "support24_7")}
            </p>
          </div>
        </div>
      </section>
